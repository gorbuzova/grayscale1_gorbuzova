...
int main(int argc, char** argv) {
    ...
    unsigned char *reference_image = stbi_load(argv[1], &ref_width, &ref_height, &ref_channels, 0);
    if (!reference_image) {
    ...
    unsigned char *my_image = stbi_load(argv[2], &my_width, &my_height, &my_channels, 0);
    if (!my_image) {
    ...
    if (ref_width != my_width || ref_height != my_height || ref_channels != my_channels) {
        printf("Images have different dimensions or channels: "
               "ref %dx%dx%d, my %dx%dx%d\n", ref_width, ref_height, ref_channels, my_width, my_height, my_channels);
        stbi_image_free(reference_image);
