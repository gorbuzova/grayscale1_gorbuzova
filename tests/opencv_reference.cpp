...
int main(int argc, char **argv) {
    if (argc < 2 || argc > 3) {
        std::cout << "Usage: " << argv[0] << " <image_path> [border_mode]" << std::endl;
        return -1;
    }
    ...
    cv::filter2D(img, blur_result, CV_32F, kernel_blur, cv::Point(-1, -1), 0, border_type);
    cv::filter2D(img, identity_result, CV_32F, kernel_identity, cv::Point(-1, -1), 0, border_type);
    cv::Mat kernel_sobel_x_flipped;
    cv::flip(kernel_sobel_x, kernel_sobel_x_flipped, -1);
    cv::filter2D(img, sobel_result, CV_32F, kernel_sobel_x_flipped, cv::Point(-1, -1), 0, border_type);
